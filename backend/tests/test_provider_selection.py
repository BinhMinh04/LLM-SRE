...
`select_analyzer` / `select_embedder` pick the adapter from config. Bedrock/Titan are the defaults;
...
from app.interface.http.deps import select_analyzer, select_embedder
...
def test_defaults_are_bedrock_and_titan():
    s = _settings()
    assert isinstance(select_analyzer(s), BedrockAnalyzer)
...
def test_deepseek_selected_when_configured():
    ...
    assert isinstance(select_analyzer(s), DeepSeekAnalyzer)
